...
namespace SubSonic.Tests
{
    ...
    public class QueryTest
    {
        ...
        /// <summary>
        /// Query_s the view.    No Invoices view.
        /// </summary>
        //[Test]
        //public void Query_View()
        //{   // There is no column named 'ShipPostalCode' in table Invoices Parameter name: columnName
        //    int counter = 0;
        //    using(
        ...
        {
            Query q = new Query("Products By Category", "Northwind");
            q.PageSize = 10;
            q.PageIndex = 1;
            q.ORDER_BY("CategoryName", "ASC");
            int counter = 0;                            // SQLite error no such table: main.Sales By Category
            using(IDataReader rdr = q.ExecuteReader())  // SQLite error near "By": syntax error
            {
                while(rdr.Read())
                    counter++;
                rdr.Close();
        ...
        public void Query_JoinedDataSet_OrderByCollection()
        {
            ...
            //Assert.IsTrue(ds.Tables[0].Rows[0]["ProductID"].Equals(73), "73 != " + ds.Tables[0].Rows[0]["ProductID"].ToString());
        ...
        public void Query_ORDER_BY()
        {
            ...
            Assert.IsTrue(ds.Tables[0].Rows[0]["ProductID"].Equals(1L), "1 != " + ds.Tables[0].Rows[0]["ProductID"].ToString());
        }
        ...
        public void QueryExecuteJoinedDataSet_ORDER_BY_DESC_Collection()
        {
            ...
            //Assert.IsTrue(ds.Tables[0].Rows[0]["ProductID"].Equals(48));      // 48 != 62
            Assert.IsTrue(ds.Tables[0].Rows[0]["ProductID"].Equals(62L), "62 != " + ds.Tables[0].Rows[0]["ProductID"].ToString());
        }
